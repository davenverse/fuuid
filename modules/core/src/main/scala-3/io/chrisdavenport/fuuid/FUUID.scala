            ...
            .fold(
              ...
              _ => {
                '{
                  ...
                  val fuuid = _root_.io.chrisdavenport.fuuid.FUUID.fromString($fs).fold(throw _, _root_.scala.Predef.identity)
                  fuuid
                }
              })
        case _ => report.throwError("""This method uses a macro to verify that a FUUID literal is valid. Use FUUID.fromString if you have a dynamic value you want to parse as an FUUID.""", pos)
      }
  }
